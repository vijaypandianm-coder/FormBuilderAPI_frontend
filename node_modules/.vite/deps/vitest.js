...
var test$3 = (val) => {
  var _a5;
  return ((_a5 = val == null ? void 0 : val.constructor) == null ? void 0 : _a5.name) && testNode(val);
};
...
function diffStrings(a, b, options) {
  ...
  if (truncate3) {
    ...
    const _a5 = aMultipleLines ? `${a.split(aNewLineSymbol, truncateThreshold).join(aNewLineSymbol)}
` : a;
...
function createMock({ state, config: config2, name: mockName, prototypeState, prototypeConfig, keepMembersImplementation, prototypeMembers = [] }) {
    ...
    var A = (r) => Array.isArray(r), d2 = (r) => typeof r == "function", Q = (r) => r.length === 0, W = (r) => typeof r == "number", K = (r) => typeof r == "object" && r !== null, X = (r) => r instanceof RegExp, b = (r) => typeof r == "string", h2 = (r) => r === void 0, Y = (r) => {
      ...
      return (e) => {
        ...
        return (t) => {
          ...
          if (c) return t.index = c.index, ((_a5 = c.output) == null ? void 0 : _a5.length) && t.output.push(...c.output), true;
          {
...
var parse = function(p) {
  var _a5, _b;
  const root = ((_b = (_a5 = _PATH_ROOT_RE.exec(p)) == null ? void 0 : _a5[0]) == null ? void 0 : _b.replace(/\\/g, "/")) || "";
...
var delimiter = (() => {
  var _a5;
  return ((_a5 = globalThis.process) == null ? void 0 : _a5.platform) === "win32" ? ";" : ":";
})();
...
function createTestContext(test5, runner2) {
  ...
  context.annotate = (message, type3, attachment) => {
    ...
    const stack = findTestFileStackTrace(test5.file.filepath, new Error("STACK_TRACE").stack);
    let location;
    if (stack) {
      location = {
        file: stack.file,
...
// node_modules/vitest/dist/chunks/utils.CG9h5ccR.js
...
function decodedMappings(map2) {
  var _a5;
  return (_a5 = cast(map2))._decoded || (_a5._decoded = decode(cast(map2)._encoded));
}
...
async function saveInlineSnapshots(environment, snapshots) {
  const MagicString = (await import("./magic-string.es-O3L5EF7G.js")).default;
...
// node_modules/vitest/dist/chunks/date.-jtEtIeV.js
var RealDate = Date;
var now2 = null;
  ...
  constructor(y, m, d2, h2, M, s, ms) {
    ...
    switch (arguments.length) {
      ...
      case 1:
        ...
        break;
      default:
        d2 = typeof d2 === "undefined" ? 1 : d2, h2 = h2 || 0, M = M || 0, s = s || 0, ms = ms || 0, date = new RealDate(y, m, d2, h2, M, s, ms);
        break;
    }
...
// node_modules/vitest/dist/chunks/vi.BZvkKVkM.js
var unsupported = [
  "matchSnapshot",
  "toMatchSnapshot",
...
function createExpectPoll(expect2) {
  return function poll(fn2, options = {}) {
    var _a5;
    const defaults = ((_a5 = getWorkerState().config.expect) == null ? void 0 : _a5.poll) ?? {}, { interval = defaults.interval ?? 50, timeout = defaults.timeout ?? 1e3, message } = options, assertion = expect2(null, message).withContext({ poll: true });
    ...
    const proxy = new Proxy(assertion, { get(target, key, receiver) {
      ...
      return function(...args) {
        const STACK_TRACE_ERROR = new Error("STACK_TRACE_ERROR"), promise = () => new Promise((resolve4, reject) => {
          let intervalId, timeoutId, lastError;
          const { setTimeout: setTimeout3, clearTimeout: clearTimeout3 } = getSafeTimers(), check = async () => {
            try {
              chai_exports.util.flag(assertion, "_name", key);
              const obj = await fn2();
              chai_exports.util.flag(assertion, "object", obj), resolve4(await assertionFunction.call(assertion, ...args)), clearTimeout3(intervalId), clearTimeout3(timeoutId);
            } catch (err) {
              if (lastError = err, !chai_exports.util.flag(assertion, "_isLastPollAttempt")) intervalId = setTimeout3(check, interval);
            }
          };
          timeoutId = setTimeout3(() => {
            clearTimeout3(intervalId), chai_exports.util.flag(assertion, "_isLastPollAttempt", true);
        ...
        let awaited = false;
        test5.onFinished ?? (test5.onFinished = []), test5.onFinished.push(() => {
          if (!awaited) {
            const negated = chai_exports.util.flag(assertion, "negate") ? "not." : "", assertionString = `expect.${chai_exports.util.flag(assertion, "_poll.element") ? "element(locator)" : "poll(assertion)"}.${negated}${String(key)}()`, error = new Error(`${assertionString} was not awaited. This assertion is asynchronous and must be awaited; otherwise, it is not executed to avoid unhandled rejections:
...
`);
        ...
        return {
          then(onFulfilled, onRejected) {
            return awaited = true, (resultPromise || (resultPromise = promise())).then(onFulfilled, onRejected);
          },
          catch(onRejected) {
...
function recordAsyncExpect2(_test2, promise, assertion, error) {
  ...
  if (test5 && promise instanceof Promise) {
    ...
    }), !test5.promises) test5.promises = [];
    test5.promises.push(promise);
    let resolved = false;
    return test5.onFinished ?? (test5.onFinished = []), test5.onFinished.push(() => {
      var _a5;
      if (!resolved) {
        const stack = (((_a5 = globalThis.__vitest_worker__) == null ? void 0 : _a5.onFilterStackTrace) || ((s) => s || ""))(error.stack);
        console.warn([
          `Promise returned by \`${assertion}\` was not awaited. `,
          "Vitest currently auto-awaits hanging assertions at the end of the test, but this will cause the test to fail in Vitest 3. ",
          "Please remember to await the assertion.\n",
...
var SnapshotPlugin = (chai, utils) => {
  ...
  for (const key of ["matchSnapshot", "toMatchSnapshot"]) utils.addMethod(chai.Assertion.prototype, key, function(properties, message) {
    ...
    const expected = utils.flag(this, "object"), test5 = getTest(key, this);
  ...
  utils.addMethod(chai.Assertion.prototype, "toMatchFileSnapshot", function(file, message) {
    ...
    const error = new Error("resolves"), expected = utils.flag(this, "object"), test5 = getTest("toMatchFileSnapshot", this), errorMessage = utils.flag(this, "message"), promise = getSnapshotClient().assertRaw({
      received: expected,
      message,
      isInline: false,
      rawSnapshot: { file },
    ...
    if (test5.each || ((_a5 = test5.suite) == null ? void 0 : _a5.each)) throw new Error("InlineSnapshot cannot be used inside of test.each or describe.each");
    ...
    const expected = utils.flag(this, "object"), test5 = getTest("toThrowErrorMatchingSnapshot", this), promise = utils.flag(this, "promise"), errorMessage = utils.flag(this, "message");
    getSnapshotClient().assert({
...
chai_exports.use(JestExtend), chai_exports.use(JestChaiExpect), chai_exports.use(SnapshotPlugin), chai_exports.use(JestAsymmetricMatchers);
function createExpect(test5) {
  const expect2 = (value, message) => {
    const { assertionCalls } = getState(expect2);
    setState({ assertionCalls: assertionCalls + 1 }, expect2);
    const assert5 = chai_exports.expect(value, message), _test2 = test5 || getCurrentTest();
  ...
  }, expect2), expect2.assert = chai_exports.assert, expect2.extend = (matchers) => chai_exports.expect.extend(expect2, matchers), expect2.addEqualityTesters = (customTesters) => addCustomEqualityTesters(customTesters), expect2.soft = (...args) => {
    return expect2(...args).withContext({ soft: true });
  }, expect2.poll = createExpectPoll(expect2), expect2.unreachable = (message) => {
            ...
            timersPromisesModule.setInterval = (delay, value, options = {}) => ({
              [Symbol.asyncIterator]: () => {
                ...
                return {
                  next: async () => {
                    var _a5, _b;
                    if (((_a5 = options.signal) == null ? void 0 : _a5.aborted) && !hasThrown) {
                      hasThrown = true;
...
var FakeTimers = class {
  constructor({ global: global3, config: config2 }) {
    ...
    this._userConfig = config2, this._fakingDate = null, this._fakingTime = false, this._fakeTimers = fakeTimersSrcExports.withGlobal(global3), this._global = global3;
  }
  clearAllTimers() {
  ...
  useFakeTimers() {
    ...
    if (((_b = (_a5 = this._userConfig) == null ? void 0 : _a5.toFake) == null ? void 0 : _b.includes("nextTick")) && isChildProcess()) throw new Error("process.nextTick cannot be mocked inside child_process");
...
function waitFor(callback, options = {}) {
  const { setTimeout: setTimeout3, setInterval: setInterval2, clearTimeout: clearTimeout3, clearInterval: clearInterval2 } = getSafeTimers(), { interval = 50, timeout = 1e3 } = typeof options === "number" ? { timeout: options } : options, STACK_TRACE_ERROR = new Error("STACK_TRACE_ERROR");
  return new Promise((resolve4, reject) => {
    ...
    checkCallback() !== true && (timeoutId = setTimeout3(handleTimeout, timeout), intervalId = setInterval2(checkCallback, interval));
  });
}
function waitUntil(callback, options = {}) {
  ...
  return new Promise((resolve4, reject) => {
    ...
    checkCallback() !== true && (timeoutId = setTimeout3(onReject, timeout), intervalId = setInterval2(checkCallback, interval));
  });
}
function createVitest() {
  ...
  const timers = () => _timers || (_timers = new FakeTimers({
    ...
    useFakeTimers(config2) {
      var _a5, _b, _c, _d;
      if (isChildProcess()) {
        if (((_a5 = config2 == null ? void 0 : config2.toFake) == null ? void 0 : _a5.includes("nextTick")) || ((_d = (_c = (_b = state().config) == null ? void 0 : _b.fakeTimers) == null ? void 0 : _c.toFake) == null ? void 0 : _d.includes("nextTick"))) throw new Error('vi.useFakeTimers({ toFake: ["nextTick"] }) is not supported in node:child_process. Use --pool=threads if mocking nextTick is required.');
      }
...
function getImporter(name) {
  ...
  return ((_a5 = parseSingleStack(stackArray[importerStackIndex + 1])) == null ? void 0 : _a5.file) || "";
}

// node_modules/vitest/dist/chunks/benchmark.DHKMYAts.js
...
var bench = createBenchmark(function(name, fn2 = noop, options = {}) {
  ...
  if (benchFns.set(task, fn2), benchOptsMap.set(task, options), !this.todo && task.mode === "todo") task.mode = "run";
});
...
var DecodedMap2 = class {
  constructor(map2, from) {
    ...
    this.version = map2.version, this.names = names || [], this._encoded = mappings || "", this._decodedMemo = memoizedState2(), this.url = from, this.resolvedSources = (sources || []).map((s) => posixResolve(s || "", from));
  }
};
...
var EvaluatedModules = class {
  ...
  getModuleSourceMapById(id) {
    ...
    let mapString = (_a5 = MODULE_RUNNER_SOURCEMAPPING_REGEXP.exec(mod.meta.code.slice(lastIndex))) == null ? void 0 : _a5[1];
...
// node_modules/vitest/dist/chunks/index.RwjEGCQ0.js
